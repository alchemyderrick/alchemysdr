...
async function searchGoogleForXProfiles(companyHandle, maxUsers = 5, offset = 0, db = null) {
  ...
  // Ensure we're authenticated to X (pass db to check employee cookies)
  await ensureAuthenticated(db);

  let page;
  try {
    page = await puppeteerManager.newPage();
  } catch (error) {
    console.error("[X-SEARCH] Failed to create new page:", error.message);
    throw new Error(`Browser initialization failed: ${error.message}. This may be due to resource constraints on Railway. Please try again in a moment.`);
  }
  ...
  try {
    ...
    // Increase timeout on cloud environments (Railway/Render) where browser may be slower
    const IS_CLOUD = !!process.env.RAILWAY_ENVIRONMENT || !!process.env.RAILWAY_STATIC_URL || !!process.env.RENDER;
    const navigationTimeout = IS_CLOUD ? 45000 : 20000; // 45s on cloud, 20s local

    try {
      // Use domcontentloaded instead of networkidle2 to avoid hanging
      await page.goto(searchUrl, { waitUntil: "domcontentloaded", timeout: navigationTimeout });
    } catch (error) {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      try {
        // Check if page is still connected before continuing
        if (page.isClosed()) {
          console.log(`[X-SEARCH] Page was closed, stopping profile checks`);
          break;
        }

        // Visit the profile page with delay to avoid rate limiting
        const profileTimeout = IS_CLOUD ? 30000 : 10000; // 30s on cloud, 10s local
        await page.goto(`https://x.com/${username}`, { waitUntil: "domcontentloaded", timeout: profileTimeout });
    ...
    if (error.message.includes("Target closed") || error.message.includes("Session closed")) {
      throw new Error("Browser connection lost. This can happen on cloud environments due to resource constraints. Please try again.");
    } else if (error.message.includes("Execution context was destroyed")) {
      throw new Error("Browser page was destroyed unexpectedly. Please try again.");
    } else if (error.message.includes("Navigation timeout")) {
      throw new Error("X.com took too long to respond. Please try again in a moment.");
    }
