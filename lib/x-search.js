 ...
 */
async function searchGoogleForXProfiles(companyHandle, maxUsers = 5, offset = 0, sessionId = null) {
  ...
  try {
    ...
    // Load authentication cookies (user-specific if sessionId provided)
    await loadAuthCookies(page, sessionId);
 ...
 * @param {string} sessionId - Optional session ID for user-specific cookies
 */
export async function searchUsersWithCompanyInBio(companyHandle, maxUsers = 5, offset = 0, sessionId = null) {
  return await searchGoogleForXProfiles(companyHandle, maxUsers, offset, sessionId);
}
