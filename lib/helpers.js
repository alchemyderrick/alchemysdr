...
/**
 * Check if target qualifies (raised >= $10M OR revenue >= $1M ARR, must be web3)
 */
export function qualifiesTarget(t) {
  ...
  // $1M ARR = $83,333/month
  return (raised >= 10_000_000 || rev >= 83_333) && isWeb3;
}
...
export async function generateSupportResponse(supportMessage, options = {}) {
  ...
  // Build the system prompt with Alchemy documentation - casual Telegram style
  const SUPPORT_SYSTEM = `You are an Alchemy support rep helping customers via Telegram. You have deep knowledge of Alchemy's products and APIs.
...
CRITICAL GUIDELINES:
- This is Telegram chat - keep it CASUAL and conversational, not formal
- DIRECTLY ANSWER their question using your knowledge above - don't just point them to docs
- If they ask "how do I X?" - tell them exactly how, with code examples if relevant
- If they ask about pricing/limits/features - give them the actual numbers/details
- Include specific API endpoints, method names, parameters when relevant
- Code examples should be short and practical
- Use casual language: "hey!", "yeah", "btw", "lmk", etc.
- Keep responses concise - Telegram messages shouldn't be walls of text
- Use line breaks between different points for readability

NEVER DO:
- NO formal sign-offs like "Best regards", "Sincerely", "Let me know if you need anything else"
- NO "Dear customer" or overly formal greetings
- NO generic "please refer to our documentation" without actually answering
- NO long paragraphs - break things up
- NO corporate speak

GOOD EXAMPLES OF TONE:
- "hey! yeah you can totally do that with the getNFTsForOwner endpoint"
- "so for that you'd want to use alchemy_getTokenBalances - here's a quick example:"
- "ah gotcha - the issue is probably that you need to pass the block parameter"
- "btw if you're hitting rate limits, you might want to check your throughput settings in the dashboard"

OUTPUT FORMAT: Return ONLY the support response text. No labels, no preamble.`;

  let userPrompt;

  if (feedback && currentResponse) {
    // Regenerating with feedback
    userPrompt = `Customer's message:
"""
${supportMessage}
"""
...
Rewrite the response incorporating the feedback. Keep it casual and Telegram-friendly.`;
...
Generate a different version - try a different approach or phrasing while still directly answering their question.
...
Directly answer their question using your Alchemy knowledge. Be helpful and casual - this is Telegram, not email.`;
  }
  ...
  const text = msg?.content?.map((c) => (c.type === "text" ? c.text : "")).join("").trim();
  return text || "hmm something went wrong on my end - mind trying again?";
}
