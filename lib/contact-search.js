...
export async function searchCompanyContacts(target, anthropic, db, nanoid, nowISO, apolloClient) {
  ...
  // Check if target needs enrichment (missing website, notes, or funding data)
  const needsEnrichment = !target.website || !target.notes || !target.raised_usd || target.raised_usd === 0;
...
2. Total funding raised in USD (if publicly available from press releases, Crunchbase, PitchBook, news articles, etc.)
3. Estimated monthly revenue in USD (if publicly available)
4. A single sentence description of what the company does

IMPORTANT: Always provide the website and description. Only provide funding/revenue if you find reliable public sources.
