...
const approveForRelayerBtn=document.createElement("button");
approveForRelayerBtn.textContent="Approve (for Relayer)";
approveForRelayerBtn.style.flex="1";
approveForRelayerBtn.className="btn-success";
approveForRelayerBtn.onclick=async()=>{
const r=await fetch("/api/drafts/"+it.id+"/approve",{method:"POST"});
const j=await r.json().catch(()=>({}));
if(!r.ok){
alert("Failed: "+(j.error||j.message||r.status));
return;
}
alert("✅ Approved! Relayer will handle it within 10 seconds.");
await refreshQueue();
await refreshSentMessages();
await refreshApprovedTargets();
};
row.appendChild(approveForRelayerBtn);
const approveBtn=document.createElement("button");
