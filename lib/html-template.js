...
<div class="card" style="background:#fef3c7;border:2px solid #fbbf24;margin-bottom:var(--space-6)">
<h3 style="margin-top:0">🔑 X Authentication</h3>
<div id="xAuthStatus" class="muted" style="margin-bottom:12px">Checking...</div>
<div style="display:flex;gap:8px;flex-wrap:wrap">
<a href="javascript:(function(){if(!window.location.hostname.includes('x.com')&&!window.location.hostname.includes('twitter.com')){alert('⚠️ You need to run this bookmarklet while on x.com!\\n\\n1. Open x.com in a new tab\\n2. Log in to X/Twitter\\n3. Click this bookmarklet again while on x.com');return;}const c=document.cookie.split(';').map(c=>{const[n,v]=c.trim().split('=');return{name:n,value:v,domain:window.location.hostname,path:'/',secure:window.location.protocol==='https:',httpOnly:false,sameSite:'Lax'};});if(!c.length){alert('⚠️ No cookies found. Log into X first!');return;}const serverUrl=prompt('Enter your SDR Console URL:','http://localhost:3000');if(!serverUrl)return;fetch(serverUrl+'/api/x-auth/save-cookies',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({cookies:c})}).then(r=>r.json()).then(d=>alert(d.ok?'✅ Success! Saved '+d.message+'\\n\\nYou can now use X discovery in SDR Console.':'❌ Error: '+d.error)).catch(e=>alert('❌ Failed: '+e.message));})();"
style="padding:8px 16px;background:#fbbf24;color:#78350f;text-decoration:none;border-radius:6px;font-weight:600;display:inline-block">
📋 Sync X Cookies
</a>
<button onclick="checkXAuthStatus()" style="font-size:13px">🔄 Check Status</button>
</div>
<div id="xAuthInstructions" class="muted tiny" style="margin-top:12px;color:#78350f">
<b>How to use bookmarklet:</b><br>
1. Open <a href="https://x.com" target="_blank" style="color:#78350f;text-decoration:underline">x.com</a> and log in<br>
2. While on x.com, click "Sync X Cookies" above<br>
3. Enter your server URL when prompted<br>
4. Come back here - your X auth will be saved
</div>
</div>
...
try{
const statusEl=document.getElementById("xAuthStatus");
const instructionsEl=document.getElementById("xAuthInstructions");
if(!statusEl)return;
statusEl.textContent="Checking...";
statusEl.className="muted";
const r=await fetch("/api/x-auth/status");
...
noteText="<br><small>✓ X discovery will use your local x-cookies.json file. Bookmarklet is only needed for Railway deployment or multi-user setup.</small>";
}else if(data.source==="bookmarklet"){
sourceText=" (bookmarklet)";
noteText="<br><small>✓ X discovery will use your synced cookies.</small>";
}else if(data.source==="environment"){
sourceText=" (environment)";
noteText="<br><small>✓ X discovery will use X_COOKIES environment variable.</small>";
}
statusEl.innerHTML="✅ Authenticated - "+data.cookieCount+" cookies"+sourceText+noteText;
statusEl.style.color="#15803d";
if(instructionsEl&&data.source==="local_file"){
instructionsEl.innerHTML="<small><b>Note:</b> You're authenticated via local file. Bookmarklet is optional.</small>";
}
}else{
statusEl.textContent="❌ Not authenticated - use bookmarklet below";
...
const pendingCount=followUpCount-sentCount;
header.innerHTML="<b>"+escapeHtml(group.contact.name)+"</b> "+
"<span class='pill' style='background:#fbbf24;color:#78350f'>follow-ups: "+followUpCount+" ("+sentCount+" sent, "+pendingCount+" pending)</span><br>"+
...
const messagesToday=sentMessages.filter(msg=>{
const msgDate=new Date(msg.updated_at);
return msgDate>=midnightLocal;
}).length;
document.getElementById('messagesSentCount').textContent=messagesToday;
}catch(e){
