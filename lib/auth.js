...
import fs from 'fs';
import { getDatabaseDir } from './database.js';
...
function getAuthDatabase() {
  ...
  // Create shared successful_messages table (global, not per-employee)
  db.exec(`
    CREATE TABLE IF NOT EXISTS shared_successful_messages (
      id TEXT PRIMARY KEY,
    ...
    CREATE INDEX IF NOT EXISTS idx_shared_messages_created ON shared_successful_messages(created_at);
    CREATE INDEX IF NOT EXISTS idx_shared_messages_submitted ON shared_successful_messages(submitted_by);
  `);

  return db;
}

// Export getAuthDatabase for use in other modules
