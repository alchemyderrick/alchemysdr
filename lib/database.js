...
`);
  ...
  // X discovery requests table (for relayer-based discovery)
  try {
    const xDiscoveryTableExists = db.prepare(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='x_discovery_requests'"
    ).get();

    if (!xDiscoveryTableExists) {
      db.exec(`
      ...
      console.log("✅ Created x_discovery_requests table");
    }
  } catch (e) {
    console.warn("⚠️ X discovery requests table migration:", e.message);
  }
