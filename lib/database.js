...
export function getDatabaseForEmployee(employeeId) {
  ...
  // Special case: Use root database for original admin account
  // This preserves existing data for the primary user
  let dbPath;
  if (employeeId === 'derrick' || employeeId === 'derrickyen') {
    dbPath = path.join(process.cwd(), "data.db");
    console.log(`📂 Database for ${employeeId} (root): ${dbPath}`);
  } else {
    // Create separate database for other employees
    const dbDir = path.join(process.cwd(), "databases", employeeId);
