...
`);
  ...
  // Migration: Clean up dismissed targets (they should be deleted, not marked as dismissed)
  try {
    const dismissedCount = db.prepare("SELECT COUNT(*) as count FROM targets WHERE status = 'dismissed'").get();
    if (dismissedCount && dismissedCount.count > 0) {
      console.log(`📝 Cleaning up ${dismissedCount.count} dismissed targets...`);
      db.exec("DELETE FROM targets WHERE status = 'dismissed'");
      console.log(`✅ Removed ${dismissedCount.count} dismissed targets`);
    }
  } catch (e) {
    console.warn("⚠️ Dismissed targets cleanup:", e.message);
  }
