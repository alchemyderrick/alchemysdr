...
export async function isAuthenticated(db = null) {
  ...
  if (db) {
    ...
    if (config) {
      hasCookies = true;
      console.log(`[X-AUTH] Found cookies in employee database (${config.value.length} bytes)`);
    }
  }

  if (!hasCookies) {
    console.log('[X-AUTH] No cookies found in any source');
    return false;
  }

  // On cloud, skip the browser-based validation - just trust that cookies exist
  // Browser validation will happen when we actually try to use them
  if (IS_CLOUD) {
    console.log('[X-AUTH] Cloud environment - skipping browser validation, cookies exist in database');
    return true;
  }

  // On local dev, do the full browser-based validation
  console.log('[X-AUTH] Local environment - performing browser-based validation');
...
export function parseBrowserCookies(cookieString) {
  ...
  console.log(`[X-AUTH] Parsing ${pairs.length} cookie pairs from browser string`);
  ...
  for (const pair of pairs) {
    ...
    if (name && value) {
      // Add cookie for both .x.com and .twitter.com domains for compatibility
      const cookieBase = {
        name: name.trim(),
        value: value.trim(),
        path: '/',
      ...
      // Also add for .twitter.com domain (X.com redirects work with both)
      cookies.push({
        ...cookieBase,
        domain: '.twitter.com'
      });
  ...
  console.log(`[X-AUTH] Created ${cookies.length} cookie entries (duplicated for x.com and twitter.com)`);
  return cookies;
}
...
export async function validateCookies(cookies, db = null) {
  if (!cookies || cookies.length === 0) {
    console.error('[X-AUTH] No cookies provided for validation');
    return false;
  }

  console.log(`[X-AUTH] Validating ${cookies.length} cookies...`);

  // Check for essential X.com cookies (auth_token is the most critical)
  const hasAuthToken = cookies.some(c => c.name === 'auth_token');
  const hasCt0 = cookies.some(c => c.name === 'ct0');

  console.log(`[X-AUTH] Cookie check - auth_token: ${hasAuthToken}, ct0: ${hasCt0}`);

  if (!hasAuthToken) {
    console.error('[X-AUTH] Missing auth_token cookie - this is required for X authentication');
    return false;
  }
  ...
  try {
    ...
    console.log('[X-AUTH] Setting cookies in browser...');
    await page.setCookie(...cookies);

    // Try to load X home page
    console.log('[X-AUTH] Navigating to X.com/home...');
    ...
    const currentUrl = page.url();
    console.log(`[X-AUTH] Final URL: ${currentUrl}`);
    ...
    console.log(`[X-AUTH] Validation result: ${isLoggedIn ? 'SUCCESS' : 'FAILED'}`);
    return isLoggedIn;
  } catch (error) {
    console.error('[X-AUTH] Cookie validation error:', error.message);
    console.error('[X-AUTH] Stack:', error.stack);
    return false;
  }
}
