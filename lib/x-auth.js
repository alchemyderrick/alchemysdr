...
// Global user cookies storage (set by server.js)
export let userXCookiesMap = null;

export function setUserCookiesMap(map) {
  userXCookiesMap = map;
}
 ...
 * Priority: 1) User session cookies, 2) X_COOKIES env var, 3) Local file
 * @param {Page} page - Puppeteer page
 * @param {string} sessionId - Optional session ID for user-specific cookies
 */
export async function loadAuthCookies(page, sessionId = null) {
  let cookies;
  let source = "unknown";

  // Priority 1: Check user session cookies (from bookmarklet)
  if (sessionId && userXCookiesMap && userXCookiesMap.has(sessionId)) {
    cookies = userXCookiesMap.get(sessionId);
    source = `session ${sessionId}`;
    console.log(`[X-AUTH] Loading ${cookies.length} cookies from user session`);
  }
  // Priority 2: Environment variable (Railway deployment)
  else if (process.env.X_COOKIES) {
  ...
  // Priority 3: Local file (Mac development)
    ...
    throw new Error("No authentication cookies found. Please use the bookmarklet, set X_COOKIES environment variable, or run authenticate() first.");
  }
