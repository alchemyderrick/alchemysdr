...
export async function isAuthenticated(db = null) {
  ...
  // On cloud environments (Railway), skip browser verification and just check if cookies exist
  // Browser automation doesn't work on Linux without display
  if (IS_CLOUD) {
    console.log(`[X-AUTH] Cloud environment detected - trusting cookie existence check`);
    console.log(`[X-AUTH] Authentication status: AUTHENTICATED (cookies found in database)`);
    return true;
  }

  // On Mac/local, verify cookies work by checking X.com access
