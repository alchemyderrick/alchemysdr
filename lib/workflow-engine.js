...
export class WorkflowEngine {
  ...
  async executeXDiscovery(options) {
    ...
    try {
      ...
      // Check if we're on Railway - if so, delegate to relayer
      const IS_CLOUD = !!process.env.RAILWAY_ENVIRONMENT || !!process.env.RAILWAY_STATIC_URL || !!process.env.RENDER;

      let xUsers;

      if (IS_CLOUD) {
        console.log(`[WORKFLOW] Step 1/4: Creating X discovery request for relayer (starting from result ${offset + 1})...`);

        // Create discovery request for relayer to process
        const requestId = this.nanoid();
        this.db.prepare(`
          INSERT INTO x_discovery_requests (id, x_handle, target_id, max_users, offset, status, created_at)
          VALUES (?, ?, ?, ?, ?, 'pending', ?)
        `).run(requestId, x_handle, target_id, max_users, offset, this.nowISO());

        console.log(`[WORKFLOW] Waiting for relayer to complete X discovery (request: ${requestId})...`);
        ...
        while (Date.now() - startTime < maxWaitTime) {
          ...
          const request = this.db.prepare(`
            SELECT * FROM x_discovery_requests WHERE id = ?
          `).get(requestId);

          if (request.status === 'completed') {
            if (request.result_json) {
              xUsers = JSON.parse(request.result_json);
              console.log(`[WORKFLOW] Relayer completed X discovery: ${xUsers.length} users found`);
              break;
            } else {
              throw new Error("X discovery completed but no results returned");
            }
          } else if (request.status === 'failed') {
            throw new Error(request.error_message || "X discovery failed on relayer");
          }
        }

        if (!xUsers) {
          throw new Error("Timeout waiting for X discovery - make sure relayer is running on your Mac");
        }
      } else {
        // Local Mac - run directly
        console.log(`[WORKFLOW] Step 1/4: Searching X for users (starting from result ${offset + 1})...`);
        xUsers = await searchUsersWithCompanyInBio(x_handle, max_users, offset, this.db);
      }
