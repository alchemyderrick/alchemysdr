  ...
  async getBrowser() {
    if (!this.browser || !this.browser.isConnected()) {
      ...
      if (IS_CLOUD || !IS_MAC) {
        ...
        // Mac-specific: Show browser window for authentication
      ...
      if (IS_CLOUD) {
        ...
        if (!chromiumPath) {
          console.log('[PUPPETEER] Searching for Chromium executable...');

          // Try which chromium
          try {
            chromiumPath = execSync('which chromium 2>/dev/null').toString().trim();
            if (chromiumPath) console.log(`[PUPPETEER] Found chromium via 'which': ${chromiumPath}`);
          } catch (e) {}
          ...
          if (!chromiumPath) {
            try {
              ...
              if (chromiumPath) console.log(`[PUPPETEER] Found chromium-browser via 'which': ${chromiumPath}`);
            } catch (e) {}
          }

          // Try finding in common Nix paths
          if (!chromiumPath) {
            try {
              chromiumPath = execSync('find /nix/store -name chromium -type f -executable 2>/dev/null | grep -E "bin/chromium$" | head -1').toString().trim();
              if (chromiumPath) console.log(`[PUPPETEER] Found chromium in Nix store: ${chromiumPath}`);
            } catch (e) {}
          }
        ...
        if (chromiumPath) {
          ...
          console.error('[PUPPETEER] Could not find Chromium executable anywhere!');
          ...
          throw new Error('Chromium not found. Please check Railway build logs.');
        }
