...
class PuppeteerManager {
  ...
  async getBrowser() {
    if (!this.browser || !this.browser.isConnected()) {
      ...
      if (IS_CLOUD) {
        // Try to find Chromium executable using which command
        try {
          const { execSync } = await import('child_process');
          ...
          try {
            ...
            try {
              ...
              // If which fails, try common Nix store path pattern
              try {
                chromiumPath = execSync('find /nix/store -path "*/bin/chromium" -type f 2>/dev/null | head -1').toString().trim();
              } catch (e3) {
          ...
          if (chromiumPath) {
            ...
            console.error('[PUPPETEER] Could not find Chromium executable');
            throw new Error('Chromium not found. Please ensure chromium is installed.');
          }
        } catch (error) {
          console.error('[PUPPETEER] Error finding Chromium:', error.message);
          throw error;
        }
