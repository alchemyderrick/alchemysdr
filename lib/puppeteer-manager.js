      ...
      if (IS_CLOUD) {
        ...
        for (const path of chromiumPaths) {
          try {
            ...
            // For wildcard paths, try to expand them
            if (path.includes('*')) {
              const { execSync } = await import('child_process');
              const expandedPath = execSync(`ls ${path} 2>/dev/null | head -1`).toString().trim();
              if (expandedPath && fs.existsSync(expandedPath)) {
                launchOptions.executablePath = expandedPath;
                console.log(`[PUPPETEER] Found Chrome at: ${expandedPath}`);
                break;
              }
        ...
        if (!launchOptions.executablePath) {
          console.error(`[PUPPETEER] Could not find Chromium in any of: ${chromiumPaths.join(', ')}`);
          throw new Error('Chromium not found. Please set PUPPETEER_EXECUTABLE_PATH environment variable.');
        }
