...
class PuppeteerManager {
  ...
  async getBrowser() {
    if (!this.browser || !this.browser.isConnected()) {
      ...
      // Force headless on Render/production, allow visible on local Mac
      const headless = IS_RENDER || (IS_PRODUCTION && !IS_MAC) ? true : false;
      ...
      if (IS_RENDER || !IS_MAC) {
        ...
        console.log("[PUPPETEER] Running in cloud/Linux mode with headless");
      } else {
      ...
      this.browser = await puppeteer.launch({
        headless: headless,
        args: launchArgs,
        userDataDir: process.env.PUPPETEER_USER_DATA_DIR || "./puppeteer-data",
