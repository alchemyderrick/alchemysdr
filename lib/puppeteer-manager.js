      ...
      if (IS_CLOUD) {
        ...
        if (chromiumPath) {
          ...
          try {
            ...
            console.log('[PUPPETEER] Contents of /nix/store:');
            console.log(execSync('ls -la /nix/store 2>&1 | head -20 || echo "Cannot read /nix/store"').toString());
          } catch (e) {
            console.error('[PUPPETEER] Could not get diagnostic info:', e.message);
          }
          throw new Error('Chromium not found on Railway. Chromium installation may have failed during build. Check Railway build logs for nixpacks errors.');
        }
