...
class PuppeteerManager {
  ...
  async getBrowser() {
    if (!this.browser || !this.browser.isConnected()) {
      ...
      // On cloud environments, use system Chromium from environment variable
      if (IS_CLOUD) {
        if (process.env.PUPPETEER_EXECUTABLE_PATH) {
          launchOptions.executablePath = process.env.PUPPETEER_EXECUTABLE_PATH;
          console.log(`[PUPPETEER] Using Chromium from env var: ${process.env.PUPPETEER_EXECUTABLE_PATH}`);
        } else {
          console.error('[PUPPETEER] PUPPETEER_EXECUTABLE_PATH not set on cloud');
          throw new Error('PUPPETEER_EXECUTABLE_PATH environment variable must be set on cloud deployments');
        }
